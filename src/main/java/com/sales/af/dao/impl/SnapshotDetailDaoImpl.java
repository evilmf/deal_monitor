...
public class SnapshotDetailDaoImpl implements SnapshotDetailDao {
	...
	Long currentSnapshotId = ((BigInteger) query.getSingleResult())
		.longValue();
	...
	for (Object row : res) {
		...
		sdt.setProductCreateDate(new Date(((Timestamp) data[3])
			.getTime()));
		...
		sdt.setSnapshotCreateDate(new Date(((Timestamp) data[15])
			.getTime()));
	...
	Map<Long, SnapshotDetailTo> products = objectMapper.readValue(
		snapshot.getSnapshot(), typeRef);
	snapshotTo.setSnapshotDetail(products);

	return snapshotTo;
    }

    public SnapshotTo getLatestSnapshotDetail() throws JsonParseException,
